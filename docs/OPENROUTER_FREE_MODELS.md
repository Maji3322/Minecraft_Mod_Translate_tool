...
> **注意**: このドキュメントには`.env`ファイル設定に関する情報が含まれていますが、`.env`ファイル設定は非推奨です。
> 
> **推奨**: アプリケーションのUI設定ダイアログ（⚙️アイコン）を使用してAPIキーとモデルを設定してください。
> UI設定の方がセキュアで、APIキーがファイルに保存されません。
...
1. アプリケーションを起動
2. ⚙️（設定）アイコンをクリック
3. [OpenRouter](https://openrouter.ai/)でAPIキーを取得
4. APIキーを入力フィールドに貼り付け
5. 「モデルを取得」ボタンをクリック
6. 検索バーで目的のモデルを検索（例：「free」で無料モデルを検索）
7. 使用したいモデルをクリックして選択
8. （オプション）フォールバックモデルを追加
9. 「保存」をクリック

### 代替方法（開発者向け）: .env ファイル設定

> **非推奨**: この方法はセキュリティ上の理由から推奨されません。UI設定を使用してください。
...
異なるモデルを試したい場合は、UI設定ダイアログで変更するか、`.env`ファイルの`OPENROUTER_MODEL`の値を変更してください。
