...
-   [ ] OpenAI API や、Gemini API を利用してより高度な翻訳を行得るようにする(API はユーザー依存)
    -   [ ] `要件 複数のLLM APIプロパイダーに対応する（まずはOpenAI API、Gemini API、Anthropic API、Ollama Local） API KEYをを、.envから読み込ませる LangChainを利用する LLM Modeとdeep-translator Modeを切り替えるトグルボタンを用意する 近くにモデル選択の欄を設ける Ollamaの場合はエンドポイントを入力する欄を設ける モデルの選択肢を自動生成（それぞれのライブラリのメソッドを利用して自動取得） 言語モデルには言語JSONファイルの内容をすべてそのまま渡す LangChainのJSONモードを利用することで、元のキー名（クラス名）を絶対に変えることなく、Valueの部分だけを翻訳させます。 システムプロンプトを入れる部分を作ります。`
