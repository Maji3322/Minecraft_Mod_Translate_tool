...
from ..utils.exceptions import FileOperationError, ResourcePackError, TranslationError, UIError
...
class MinecraftModTranslatorApp:
    ...
    def initialize_ui(self, page: ft.Page) -> None:
        ...
        version_dropdown = styles.create_dropdown(
            "MODの対応バージョン",
            config.VERSION_TO_PACK_FORMAT.keys(),
        ...
        self.clipboard_button = styles.create_button(
            ...
            on_click=lambda _: components.select_file_from_clipboard(page, self.process_files),
            visible=False
        )
        ...
        page.on_resize = lambda e: self._handle_window_resize(e, page)
    ...
    def _handle_file_selection(self, e, page: ft.Page) -> None:
        ...
        if e.files:
            ...
            if not file_paths:
                ...
                components.show_error_dialog(page, "エラー", "有効なjarファイルが見つかりませんでした。")
                return
    ...
    def process_files(self, file_paths: List[str], file_names: List[str], page: ft.Page) -> None:
        """
        ...
        try:
            ...
            if not en_us_json_files:
                components.show_error_dialog(page, "エラー", "en_us.jsonファイルが見つかりませんでした。")
                return
            ...
            if not resource_pack.generate_resource_pack(en_us_json_files, page):
                components.show_error_dialog(page, "エラー", "リソースパックの生成に失敗しました。")
                return
            ...
            if not lang_files_to_translate:
                components.show_dialog(page, "完了", "翻訳対象のファイルが見つかりませんでした。")
            ...
            components.show_error_dialog(page, "エラー", f"処理中にエラーが発生しました: {str(e)}")
            sys.exit(1)
        except Exception as e:
            logger.error(f"Unexpected error: {e}", exc_info=True)
            components.show_error_dialog(page, "エラー", f"予期せぬエラーが発生しました: {str(e)}")
            sys.exit(1)
    ...
    def _find_language_files(self) -> List[str]:
        ...
        try:
            ...
            en_us_json_files = [
                path for path in
                glob.glob(os.path.join(config.TEMP_DIR, "**", "en_us.json"), recursive=True)
                if path
            ]
    ...
    def _clean_temp_directory(self, en_us_json_files: List[str]) -> None:
        ...
        try:
            ...
            for en_us_json_file in en_us_json_files:
                ...
                ja_jp_file = os.path.join(os.path.dirname(en_us_json_file), "ja_jp.json")
